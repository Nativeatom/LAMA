    ...
    while stopping_condition:
        ...
        if args.split_sentence:
            ...
            if "ANS=" in text:
                do_lower_case = False
                ans_pattern = "(.*)ANS=(.*)"
                answer = re.match(ans_pattern, text).groups()
                text, answer = answer
                text = text.strip()
                for lm in args.models_names:
                    # TODO: compatible with other LMs
                    tokenizer = BertTokenizer.from_pretrained(
                        "bert-base-cased", do_lower_case=do_lower_case
                    )
                    tokens = tokenizer.tokenize(answer)
                    token_ids = tokenizer.convert_tokens_to_ids(tokens)
                    answer_ids.append(token_ids)
        ...
        for model_name, model in models.items():
            ...
            if masked_indices and len(masked_indices) > 0:
                pdb.set_trace()
                evaluation_metrics.get_ranking(filtered_log_probs_list[0], masked_indices, model.vocab, index_list=index_list, ans_ids=answer_ids[0])
            pdb.set_trace()
